...
/* ------------------------------------------------------------------ */
function SlotPickerModal({ members, onSelect }) {
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4 border border-slate-100">
        ...
        <div className="flex justify-end gap-2 pt-2">
          <button
            ...
            className="w-full bg-indigo-600 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-indigo-700 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed transition duration-200"
          >
            Join Team
          </button>
        </div>
      ...
      {isCloud && teamReady && !me && (
        <SlotPickerModal
          ...
          onSelect={(chosen) => {
            ...
            setMe(chosen);
            saveSlot(chosen.slot, { name: chosen.name });
          }}
        />
      )}
